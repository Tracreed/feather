...
fn place_block(
    ...
    if chunk_entities
        ...
        .iter()
        .any(|_entity| {
            let prev_entity_position = *game.get::<PreviousPosition>(*_entity).unwrap();
            let entity_position = game.get::<Position>(*_entity).unwrap();
            //let entity_velocity = game.get::<Velocity>
            println!("prev_position: {:?}, current_position: {:?}", prev_entity_position, *entity_position);
